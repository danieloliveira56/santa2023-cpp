...
std::ostream& operator<<(std::ostream& os, move_ids_t arr) {
...
template <std::size_t Size>
mapping_t<Size> Permutation<Size>::operator*(const mapping_t<Size>& other) const {
	mapping_t<Size> result_mapping;
	for (int i = 0; i < Size; ++i) {
		result_mapping[i] = _mapping[other[i]];
	}
...
template <std::size_t Size>
mapping_t<Size> operator*(const Permutation<Size>& map1, const mapping_t<Size>& map2) {
	return map1.mapping() * map2;
}
template <std::size_t Size>
mapping_t<Size> operator*(const mapping_t<Size>& map1, const Permutation<Size>& map2) {
	return map1 * map2.mapping();
...
template <std::size_t Size>
mapping_t<Size> Permutation<Size>::inverse_map() const {
	return getInverse(_mapping);
}
...
//template <std::size_t Size>
//Puzzle<Size>::ComponentSolveOrder() {
//	_component = std::vector<std::vector<int>>(1);
//	for (int i = 0; i < Size; ++i) {
//		_component[0].push_back(i);
//	}
...
template <std::size_t Size>
int Puzzle<Size>::countMismatches(const mapping_t<Size>& map) const {
	int ctMistakes = 0;
	for (size_t i = 0; i < Size; i++)
	{
		if (_current[map[i]] != _solution[i]) {
			ctMistakes++;
		}
	}
	return ctMistakes;
...
int Puzzle<Size>::countMismatches(const Permutation<Size>& p, int component_idx) const {
	return countMismatches(p.mapping(), component_idx);
}


template <std::size_t Size>
int Puzzle<Size>::countMismatches(const mapping_t<Size>& map, int component_idx) const {
	int ctMistakes = 0;
	for (size_t i : _component[component_idx])
	{
		if (_current[map[i]] != _solution[i]) {
...
int Puzzle<Size>::countMismatches(const Permutation<Size>& p, std::vector<int> component_idxs) const {
	return countMismatches(p.mapping(), component_idxs);
}

template <std::size_t Size>
int Puzzle<Size>::countMismatches(const mapping_t<Size>& map, std::vector<int> component_idxs) const {
	int ctMistakes = 0;
